...
const ModalEditFile = (props:any)=>{
    const [folderId, setFolderId] = useState(props.folderId)
    const [selected, setSelected] = useState(props.fileName)
    ...
    useEffect(()=>{
        ...
        // console.log(selected.file_name, "selected..")
    ...
    const handleUpload = async ()=>{
        ...
        allFormData?.append('folder_id', "")
        setIsSubmitting(true)
        await apiService.fileManager.edit(allFormData, props.fileId).then(data=>{
            // console.log(data)
            setSelectedFiles([])
            setTimeout(()=>{
        ...
        <div className={`${props.show ? 'block' : 'hidden'} absolute top-0 left-0 bottom-0 bg-black/25 w-full h-full p-3 justify-center flex items-center`}>
            <div className="bg-white rounded-lg p-3 w-1/2">
                <div className="flex justify-between">
                    <h2 className="font-bold">Edit File - {props.fileName}</h2>
                    <button
